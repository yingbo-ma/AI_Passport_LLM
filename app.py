...
def create_pdf(chat_history):
    ...
    pdf.setFillColor(colors.black)
    if y - 200 < margin:  # Adjusted for larger text box height
        pdf.showPage()
        y = 750
    pdf.drawString(margin, y - 20, "Please comment your answers to the last assignment below:")

    # Increase the height of the text box
    text_box_height = 60  # Significantly increased height to accommodate more text

    pdf.rect(margin, y - (text_box_height + 40), 400, text_box_height, fill=1)

    # Add a multi-line text box with large space
    pdf.acroForm.textfield(
        name='comment',
        tooltip='Enter your comments',
        x=margin, y=y - (text_box_height + 40),
        width=400, height=text_box_height,
        borderStyle='inset',
